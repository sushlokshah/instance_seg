...
class InitialBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bias=False, relu=True):
        ...
        self.main_branch_conv1 = nn.Conv2d(
            in_channels,
            out_channels // 2,
            kernel_size=3,
            stride=1,
        ...
        self.main_branch_bnorm = nn.BatchNorm2d(out_channels // 2)
        self.main_branch_conv2 = nn.Conv2d(
            out_channels // 2,
            out_channels - 3,
            kernel_size=3,
            stride=2,
    ...
    def forward(self, x):
        main = self.main_branch_conv1(x)
        main = self.main_branch_bnorm(main)
...
if __name__ == "__main__":
    ...
    torchsummary.summary(model, (3, int(540 * 720 / 960), 720))
    iterations = 1
    start = time.time()
    for i in range(iterations):
        # print(model)
        output = model(torch.randn(8, 3, int(540 * 720 / 960), 720).cuda())
        loss = output.sum()
        loss.backward()

    end = time.time()
    print("Time taken for {} iterations is {}".format(iterations, end - start))
    print("FPS", iterations / (end - start))
