...
class InitialBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bias=False, relu=True):
        ...
        self.main_branch = nn.Conv2d(
            in_channels, out_channels - 3, kernel_size=3, stride=2, padding=1, bias=bias
        )
...
if __name__ == "__main__":
    model = ENet(num_classes=50).cuda()
    import time
    import torchsummary

    torchsummary.summary(model, (3, 540, 960))
    iterations = 1
    start = time.time()
    for i in range(iterations):
        # print(model)
        output = model(torch.randn(16, 3, 540, 960).cuda())
